...
Class Prestamo
{
	...
	{
		$sql="SELECT p.idprestamo,p.idlibro,l.titulo as libro,p.idestudiante,e.nombre as estudiante, DATE(p.fecha_prestamo) as fecha_prestamo,DATE(p.fecha_devolucion) as fecha_devolucion,p.cantidad,p.observacion,p.estado FROM prestamo p INNER JOIN libro l ON p.idlibro=l.idlibro INNER JOIN estudiante e ON p.idestudiante=e.idestudiante ORDER BY p.idprestamo desc";
		return ejecutarConsulta($sql);		
	}
